...
@Component({
  selector: "app-module-show-notification-comment",
  templateUrl: "./module-show-notification-comment.page.html",
  styleUrls: ["./module-show-notification-comment.page.scss"]
})
export class ModuleShowNotificationCommentPage implements OnInit {
  userid: any;
  notificationid: any;
  getcommentdata: any;
  sendtext: any;
  ...
  async Addcomment() {
    if (this.sendtext) {
      ...
      // this.constant.LoadingPresent();
      this.service.getCommentadd(senddata).subscribe(
        result => {
          // this.constant.LoadingHide();
          if (result.status) {
            // console.log(result.data);
      ...
      this.constant.ToastCustom("Please Enter message", "top");
    }
  }
}
