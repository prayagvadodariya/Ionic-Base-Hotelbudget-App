...
@Component({
  selector: 'app-export-capital-invoices-list',
  templateUrl: './export-capital-invoices-list.page.html',
  styleUrls: ['./export-capital-invoices-list.page.scss'],
})
export class ExportCapitalInvoiceListPage implements OnInit {
  userData: any;
  invoicedata: any;
  hotelid: any;
  ...
  getCategotytList(){
    ...
    this.service.getExportCapitalSubInvoices(this.param).subscribe((result) => {
      this.constant.LoadingHide();
      // console.log("1",result);
      if (result.status) {
        this.SubInvoiceRecordArr = result.data.CapitalSubInvoiceRecord;
  ...
  InfinitScrolling(event) {
    ...
    this.constant.LoadingPresent();
    this.service.getExportCapitalSubInvoices(this.param).subscribe(
      result => {
        this.constant.LoadingHide();
